...
def fisher_loss(tensor, batch_person, person_file_num):
    crossloss = 0
    ...
    avg_center_loss = tf.reduce_mean(tf.sqrt(tf.reduce_sum(center_dis, 2)))
    center_dis = tf.sqrt(tf.reduce_sum(center_dis, 2, True))
    center_dis = tf.reshape(center_dis, (batch_person, person_file_num))
    top_k, index = tf.nn.top_k(input=center_dis, k=1)
    center_top_k = tf.reduce_mean(top_k)
    centerloss = center_top_k + avg_center_loss
    for i in range(batch_person):
        center_i = tf.gather(center, i)
        cross_dis = -tf.sqrt(tf.reduce_sum(0.5 * (tensor - center_i)**2, 2))
        cross_dis_1D = tf.reshape(cross_dis, (-1, batch_person * person_file_num))
        value, index = tf.nn.top_k(cross_dis_1D, 1)
